...
public sealed class ObjectPool : MonoBehaviour
{
	...
	Dictionary<string, List<GameObject>> pooledObjects = new Dictionary<string, List<GameObject>>();
	...
	{
		if (prefab != null && !instance.pooledObjects.ContainsKey(prefab.name))
		{
			var list = new List<GameObject>();
			instance.pooledObjects.Add(prefab.name, list);
	...
	public static GameObject Spawn(GameObject prefab, Transform parent, Vector3 position, Quaternion rotation)
	{
		...
		Transform trans;
		GameObject obj;
		if (instance.pooledObjects.TryGetValue(prefab.name, out list))
		{
			obj = GetAvailableObjectFromList(list);
		...
		{
			CreatePool(prefab, 1);
			obj = GetAvailableObjectFromList(instance.pooledObjects[prefab.name]);
			obj.SetActive(true);
		}
	...
	static GameObject GetAvailableObjectFromList(List<GameObject> availableList)
	{
		if (availableList.Count <= 0)
			return null;

		GameObject obj = null;
		while (obj == null && availableList.Count > 0)
		{
			obj = availableList[0];
	...
	{
		instance.pooledObjects[prefab.name].Add(obj);
	...
	public static List<GameObject> GetPooled(GameObject prefab, List<GameObject> list, bool appendList)
	{
		...
		if (instance.pooledObjects.TryGetValue(prefab.name, out pooled))
	...
	public static List<T> GetPooled<T>(T prefab, List<T> list, bool appendList) where T : Component
	{
		...
		if (!appendList)
			list.Clear();
		List<GameObject> pooled;
		if (instance.pooledObjects.TryGetValue(prefab.gameObject.name, out pooled))
